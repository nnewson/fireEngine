...
- **GPU soft-body / cloth (XPBD)** — `-c` drops a cloth that simulates entirely on the GPU. A renderer-owned `SoftBodySystem` runs an XPBD compute solver each substep: `cloth_predict` integrates gravity + wind, `cloth_solve` projects distance constraints **graph-coloured** into race-free batches (Gauss-Seidel by colour), `cloth_collide` pushes particles out of world colliders, and `cloth_finalize` writes solved positions + grid-recomputed normals into a storage **vertex buffer** the forward/shadow passes read — so the cloth renders, lights, and casts shadows through the normal forward path (double-sided), no new render shaders. Collision primitives (plane / sphere / box / capsule) are gathered each frame from `PhysicsWorld` (`gatherColliders`) plus a ground plane and uploaded to a per-frame UBO. Substeps, compliance (stiffness), damping, gravity, and wind are **live overlay sliders**. The demo cloth drapes over a sphere onto the floor. Built on the same compute + buffer-barrier path as particles
