...
**1.1 [B] Opaque draws are unsorted.** `buildDrawBuckets` (`renderer.cpp:562`) depth-sorts
...
**1.2 [C] Per-barrier `pipelineBarrier2` calls.** Every `forwardImageBarrier` /
...
**1.3 [B] Deformation runs in every pass's vertex shader.** A skinned+morphed mesh runs full
...
**1.4 [C] `view_` uses a fixed world-up.** `drawFrame` builds
...
**1.5 [C] `LightUBO` is uploaded twice per frame.** `updateLightData` writes the full UBO
...
slots) — all micro, listed for completeness.
...
**2.1 [B] The shadow pass has a fixed per-frame floor independent of content.**
...
**2.2 [B] The world-only CSM is fully redundant when no skinned mesh exists.**
...
**2.3 [B/C] No punctual-shadow change detection.** Spot/point casters re-render all faces
...
**2.4 [A] TAA history index assumes exactly two frames in flight.**
...
**2.5 [C] The resolve→blit round trip costs a full-res HDR copy per frame.** The resolve
...
**2.6 [—] Shadow LOD is discrete while forward is VIPM/VDPM.** Already documented as a known
...
**3.1 [B] Per-step heap allocation in the hot path.** Every `step()` allocates:
...
**3.2 [C] Duplicated magic sleep threshold.** The jointed-island angular sleep threshold
...
**3.3 [C] Mesh-triangle warm-start key mixing can collide.**
...
**3.4 [C] Hinge axis-alignment rows are prepare-frozen.** `addAxisRows` computes the
...
O(front).** Per instance per frame, the CPU path runs: a full `O(splits)` score scan
...
**4.2 [C] The chart veto uses static chart sets.** `canonicalCharts_` is computed once from
...
**4.3 [C] Simplifier build-time allocation churn.** `collapse()` allocates `oneRing`,
...
the right tooling to keep them observable.

---

## 5. Cross-cutting observations

**5.1 [B] The renderer records everything, every frame.** There is no notion of "nothing
...
**5.2 [C] Barrier/pass micro-structure vs MoltenVK.** Several findings (1.2, 2.1, 2.5)
