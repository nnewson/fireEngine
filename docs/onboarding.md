...
- `extras.Shadow` — `{"Casts": false}` makes the node's mesh a shadow **receiver only**
  (`Geometry::castsShadow(false)`). Needed by any wide flat receiver: a floor that casts writes its
  own depth into every cascade, fails its own depth comparison across its whole area, and greys the
  scene out — the built-in `-f` plane has always set this in code, and this is how an authored scene
  says it (`assets/shadow_lod/ShadowLodDemo.gltf`). `Casts` is required inside the block, and it is
  the only key — any other key, including `Receives`, is **rejected**, not ignored. **Granularity:**
  the flag is per *instance*, stored on `Object::GeometryBindings`, not on the shared `Geometry`. Two
  nodes may instance one mesh as a caster and as a receive-only surface and both are honoured; a
  flag on the geometry could not express that, since the loader builds one `Geometry` per mesh.
...
- `src/core/gltf_loader_extras.cpp` — `extras.Controllable`/`Physics`/`Cloth`/`Ragdoll`/`Shadow` parsing
...
- `--capture <path.png>` / `--capture-frame N`: write the numbered frame's final swapchain content
  ...
  advance on wall-clock `dt`, so identical CONTENT needs a static scene. Used to regenerate the
  shadow-LOD reference
  images; see [`acceptance-testing.md`](acceptance-testing.md). Requires swapchain `TRANSFER_SRC`,
...
- Generated glTF assets: `tools/assetgen/` (geometry primitives, quaternion helpers, a deterministic PNG writer, the self-contained-glTF `Scene` assembler). Generator scripts own their scene content and import the mechanics from here; anything added must keep committed assets byte-identical, since the build regenerates them.
- Shadow-LOD acceptance scenes: `assets/shadow_lod/` (`ShadowLodDemo.gltf` static baseline + `ShadowLodMotionDemo.gltf` motion loop, generated with structural validation). The SH-01 evidence scene — see [`acceptance-testing.md`](acceptance-testing.md) § Shadow-LOD acceptance scenes for what to look at and which known exposures belong to which later item.
- Physics demos: `assets/physics_demos/` (one glTF per capability, generated by `generate.py` on top of `tools/assetgen/`); behaviour tests in `tests/physics/test_demos.cpp` (`[Demos]`). See [`collision.md`](collision.md) § Physics demos and [`README.md`](../README.md) § Physics Demos for the run commands + what each shows.
