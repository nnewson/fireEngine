...
| File | Pay attention to |
|---|---|
| `render/device.hpp` + `device.cpp` | Instance/surface/physical+logical device/queues, and the process-wide **VMA allocator** (`allocator()`, owned here as a `VmaAllocatorHandle`; buffers/images sub-allocate from it — `createBuffer` returns a `UniqueVmaBuffer`). Built with `VULKAN_HPP_NO_CONSTRUCTORS` — designated initializers everywhere. Device selection: `missingDeviceCapabilities` verifies the whole enabled feature chain (1.0/1.2/1.3 + portability-subset) + descriptor-indexing update-after-bind limits during `isDeviceSuitable`, so an unsuitable GPU is rejected with a named reason; `createLogicalDevice`'s enable-chain must stay in lockstep with that list. |
