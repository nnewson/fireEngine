...
| `graphics/object.hpp` + `src/graphics/object.cpp` | **High-attention (~560 lines).** Writes mapped UBOs, **selects alpha pipeline from material state** (but only returns a `PipelineHandle` — binding happens later), emits `DrawCommand`s. Also runs the **per-draw LOD** work: `selectLod` (discrete, forward + coarser for shadows) / `selectVipm` (Continuous morph uniforms) / the per-instance VDPM `refineForView` + `repairCoverage` → per-frame dynamic index buffer (ViewDependent takes precedence). Sets `cmd.indexBuffer`/`indexCount`/`lodLevel`. Watch the per-`currentFrame` mapped-UBO/index writes. |
