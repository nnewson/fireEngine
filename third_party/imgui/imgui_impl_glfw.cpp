...
// Fire Engine local patch: GLFW's Cocoa native header pulls in CoreGraphics /
// Objective-C (block `^` syntax, mach asserts), which g++ can't parse under the
// current macOS SDK. We don't use native window handles (no multi-viewport), so
// the include is skipped and PlatformHandleRaw is left null below. Re-apply this
// edit if the backend is re-vendored from a newer ImGui.
// #include <GLFW/glfw3native.h>
#elif GLFW_HAS_X11
...
#elif defined(__APPLE__)
    main_viewport->PlatformHandleRaw = nullptr; // Fire Engine: Cocoa native header skipped (see top)
#else
